...
@SuppressWarnings({ "PMD.SignatureDeclareThrowsException", "PMD.LawOfDemeter", "removal", "PMD.ExcessiveImports",
		"PMD.CouplingBetweenObjects", "squid:S4502" })
public class WebSecurityConfig {
	...
	private static final String[] AUTH_WHITELIST = { "/health/**", "/error**", "/fonts/glyph*", "/css/xhibit.css",
			"/css/bootstrap.min.css", "/js/bootstrap.min.js", "/WEB-INF/jsp/error**", "/css/**", "/js/**",
			"favicon.ico", "/login**", "/oauth2**", "/default-ui.css", "/logout**", "/WEB-INF/jsp/logon**" };
	...
	protected HttpSecurity getAuthClientHttp(HttpSecurity http) throws Exception {
		http.oauth2Login(auth -> auth.successHandler(getSuccessHandler()).failureHandler(getFailureHandler())
				.authorizationEndpoint(
						endPoint -> endPoint.authorizationRequestRepository(cookieAuthorizationRequestRepository())))
				.addFilterAfter(new AuthorisationTokenExistenceFilter(), SecurityContextHolderFilter.class)
	...
	public AuthenticationSuccessHandler getSuccessHandler() {
		return new AuthenticationSuccessHandler() {
			...
			public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
				...
				LOG.info("The user {} has logged in.", AuthorizationUtil.getUsername(authentication));

				OidcIdToken token = AuthorizationUtil.getToken(authentication);
				cookieAuthorizationRequestRepository().saveAuthorizationToken(token, request, response);
				cookieAuthorizationRequestRepository().saveUsername(AuthorizationUtil.getUsername(authentication),
						request, response);
	...
	public AuthenticationFailureHandler getFailureHandler() {
		return new AuthenticationFailureHandler() {
			...
			public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response,
				...
				String errorMsg = stacktrace.isEmpty() ? exception.getMessage() : stacktrace.get(0).toString();
	...
	public final class AuthorisationTokenExistenceFilter extends OncePerRequestFilter {
		...
		protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
			...
			OidcIdToken token = cookieAuthorizationRequestRepository().loadAuthorizationToken(request);
			if (token != null) {
